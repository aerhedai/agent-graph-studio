...
3. **Confirm a GitHub issue exists for this spec — create one if it doesn't.** Don't assume `/start-spec` was run first. Search for it (`gh issue list --search "Implement SPEC-NNN in:title" --state all`); if nothing matches, create it using the same shape `/start-spec` uses (extract the spec's `## Acceptance criteria` checklist verbatim, `gh issue create --title "Implement SPEC-NNN: <title>" --body "Spec: docs/specs/<filename>\n\n## Acceptance criteria\n<checklist>"`), add it to the project board, and set its status to "In progress" (matching a spec whose branch already exists and is actively being worked). Report the issue number/URL either way — found or created — before continuing.

4. **Use plan mode.** Do not write any code yet. Produce a plan that explicitly:
...
5. **Once approved, implement in the phases you proposed**, pausing between phases for me to check in if the plan called for that.

6. **Before reporting anything as complete**:
...
7. Do not commit or push anything as part of this command — leave the working directory as-is with changes ready for me to review. Committing happens after I've verified the work myself.
