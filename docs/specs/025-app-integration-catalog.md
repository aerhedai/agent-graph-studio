...
- [ ] `git diff main -- backend/execution/engine.py` is empty.
  - **Resolved during implementation, superseding this criterion:** literal-value input slots (Phase 0) turned out to be a genuine prerequisite for dynamic option loading (Phase 5) — generated MCP node parameters were edge-only with no way to attach a live dropdown at all. Explicitly asked and decided this session in favor of building literal-value input slots as a real prerequisite, over scoping dynamic options to config-fields-only. This requires one minimal, justified `gather_inputs` change in `backend/execution/engine.py` (a literal-value fallback when no edge is present, edge still always wins) — shown and explained in the implementing PR, not silently included. This criterion as originally written no longer holds; superseded by the above.

## 7. Open questions

- **Exact shape of the "catalog bootstrap" admin token**: does the admin use *their own* real personal account's OAuth connect to seed the schema (simplest, matches how `my-gmail` was actually bootstrapped this session), or is a separate, disposable "discovery-only" credential worth supporting for apps where the admin doesn't want to personally connect their own account just to seed the catalog?
  - **Resolved: admin's own connect is sufficient.** `POST /connections/{name}/catalog-bootstrap` requires the admin to already hold a valid personal credential (OAuth token or api_key) for the connection, and uses it directly — no separate disposable discovery-only credential type was built. Live-verified against a real external MCP server (Context7): an admin's own stored credential bootstrapped global node types that a second, never-connected user immediately saw in their palette.
- **Should `connection_shares` (direct one-connection-to-specific-other-user sharing) be a fast-follow to this spec, or wait until a concrete use case shows the existing shared-graph-slot model isn't enough?**
  - **Resolved: wait.** Not built as part of this spec — no concrete use case surfaced during implementation that the existing shared-graph-slot model (SPEC-021) doesn't already cover. Revisit if one does.

## 8. Implementation notes (post-implementation)

- **Phase 2 (`api_key`/`bearer` auth)** and **Phase 6 (a second real OAuth app)** each have one acceptance criterion that could not be live-verified end-to-end in the implementing session: both require a real, user-owned external credential (a genuine Apify-style API key; a real OAuth app registered in Linear/Atlassian/Stripe/Zoom/Figma's own console) that only the project owner can obtain. The underlying mechanism for both is otherwise fully implemented and covered by real (non-mocked-business-logic) HTTP-level tests, matching this project's existing precedent for OAuth-handshake testing (`tests/test_mcp_oauth_connections_api.py` mocks only the external provider calls, not the route logic). Genuinely live, non-mocked, real-external-server verification *was* performed for Phases 0, 1, 3, and 5 against Google's real Gmail MCP server (earlier work) and Context7's real public MCP server (`https://mcp.context7.com/mcp`, no credential required) — see the implementing PR for the actual commands/output.
- **Phase 4 (popup-based OAuth UX)**: the server-side mechanism (state-token-carried `popup` flag, postMessage-and-close callback page) is implemented and covered by real HTTP-level tests asserting the actual rendered page content. The one thing not verified is a real browser actually opening the popup, receiving the message, and closing it — no browser-automation tooling was available in the implementing session. Worth a manual click-through before this ships to end users.
